...
- **Acción:**
  - [x] Dejar **un único `btn-primary` visible** en el Dashboard: el "Hazlo mío" de la Oportunidad #1 (`opportunityHTML`).
  - [x] Bajar a `btn-secondary` el botón del whale (`whaleHTML`) y el "Desarrollar" del idea-input (`ideaInputHTML`) cuando se renderizan dentro del Dashboard.
  - [x] Convertir `voiceOnboardCardHTML` en un **banner delgado** (no una card con CTA primario) y colocarlo de forma que **no empuje la Oportunidad #1 por debajo del fold**. Mantener su CTA como `btn-secondary`.
  - [x] Aplicar el mismo criterio a `nextSeriesHTML` (CTA secundario en el Dashboard; puede seguir primario en el Cerebro, donde es la acción principal).
...
- **Acción:**
  - [x] Crear un helper reutilizable `promptSheet({title, label, placeholder, helper, multiline, validate, onSubmit})` que renderice un overlay con `overlayShellHTML` + primitivos `.field` (label + input/textarea + helper + error).
  - [x] Migrar los 5 flujos a `promptSheet`. "Refinar voz" debe usar **textarea** (igual que el onboarding) y la misma ruta de backend.
  - [x] Validación mínima por flujo (URL no vacía / con http; email con `@`; usuario IG sin `@`).
...
- **Acción:**
  - [x] Añadir un listener `keydown` global (engancharlo en `mount`, junto al de `click`).
  - [x] `Escape` cierra el overlay activo (gen/script/result/perf/prompter/fillweek), equivalente a `close-feed`/`back` según el caso.
  - [x] `Enter` en `#rsIdeaSeed` dispara `seed-go`; en `#rsIdeaSeed2`, `seed-add`. (No interceptar Enter dentro de textareas.)
  - [x] `⌘K`/`Ctrl+K`: enfocar un buscador **real**, o —si no se va a implementar ahora— **eliminar el `<span>⌘K</span>`** del searchbox para no prometer un atajo inexistente.
...
- **Acción:**
  - [x] Añadir `role="status"` + `aria-live="polite"` al nodo `#rsToast`.
  - [x] Separar `showToast(msg)` (info, auto-oculta 2,8 s) de `showError(msg)` (persistente, con botón de cerrar, `aria-live="assertive"`).
  - [x] Enrutar los toasts de error de red/fallo (`onboardVoice`, `refineVoice`, `linkReelPublished`, `refreshReels`, `igConnectProfile`) a `showError`.
...
- **Acción:**
  - [x] `overlayShellHTML` y el teleprompter renderizan con `role="dialog"`, `aria-modal="true"` y `aria-label` (el título del overlay).
  - [x] Al abrir un overlay, mover el foco al primer control (o al botón de cerrar). Al cerrar, devolver el foco al elemento que lo abrió.
  - [x] Trap de foco básico dentro del overlay (Tab no escapa al fondo).
...
- **Acción:**
  - [x] A partir de ~8 s sin respuesta, mostrar un mensaje honesto en el orbe ("Tu rival hablaba mucho, dame unos segundos más…").
  - [x] Ofrecer "seguir navegando" mientras se genera en segundo plano (o, mínimo, un estado de progreso distinto del bucle de pasos).
...
- **Acción:**
  - [x] Convertir `--text-xs … --text-5xl` de `px` a `rem` (base 16px). Mantener los valores visuales.
  - [x] Reflejar el cambio en `static/design-system.html` (su comentario ya avisa de mantenerlo sincronizado).
...
- **Acción:**
  - [x] Asegurar que "explosivo/alto" (`.row-score.hi`) y los badges se distingan por **icono o etiqueta**, no solo por color.
  - [x] Revisar que los targets táctiles en `rs--mobile` lleguen a **≥44px** (chips a 28px y `btn-sm` a 32px se quedan cortos en móvil).
...
- **Acción:**
  - [x] Tras descartar, mostrar un toast con acción **"Deshacer"** que revierta `status` a `draft` (y, en prod, persista con `persistRecStatus`).
...
- [x] `node --check static/js/radar-loop.js` en verde.
- [x] Harness funcional monta la isla y renderiza feed sin errores (demo) — ver `docs/PLAN-radar-loop-rediseno.md` §7.
- [x] `grep -n "prompt(" static/js/radar-loop.js` → 0 resultados.
- [x] Recorrido manual demo: despertar → Hazlo mío → reveal → grabar; probar `Esc`, `Enter`, foco en overlays, toast de error persistente.
- [ ] Recorrido con plan real (prod, `isDemo()` falso): un robo, una idea, conectar IG, vincular reel — sin `window.prompt`. *(Pendiente: el entorno local no tiene claves Supabase — `.env` con SUPABASE_URL vacío. Garantía estática: `grep prompt(` → 0 y las ramas prod usan los mismos sheets. Verificar en el próximo deploy a prod o local con claves.)*
- [x] Probar deep-links del demo (`?plan=creador`/`?plan=agencia`, `?t=`, `?b=`) — todas las vistas siguen montando.
